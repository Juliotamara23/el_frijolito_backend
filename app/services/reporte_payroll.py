...
SELECT
    ...
    -- Tipo de Recargo
    COALESCE(
        STRING_AGG(DISTINCT tr.tipo_hora, '\n') 
        FILTER (WHERE tr.tipo_hora IS NOT NULL), 'Sin recargos'
    ) AS tipo_recargo,

    -- Cantidad de días
    COALESCE(
        STRING_AGG(DISTINCT qv.cantidad_dias::text || ' días', '\n') 
        FILTER (WHERE qv.cantidad_dias IS NOT NULL), '0 días'
    ) AS cantidad_dias,

    -- Valor quincenal
    COALESCE(
        STRING_AGG(DISTINCT '$ ' || qv.valor_quincena::text, '\n') 
        FILTER (WHERE qv.valor_quincena IS NOT NULL), '$0'
    ) AS valor_quincena,
